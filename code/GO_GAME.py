...
class Application(Tk):
    ...
    def __init__(self, my_mode_num=9):
        ...
        # 画布控件，作为容器
        self.canvas_bottom = Canvas(self, bg='#708090', bd=0, width=600 * self.size, height=400 * self.size)
        self.canvas_bottom.place(x=0, y=0)

        GAME_BUTTON_FONT = ("华文行楷", 20)
        # 几个功能按钮
        self.startButton = Button(self, text='开始游戏', height=1, width=10, font=GAME_BUTTON_FONT, command=self.start)
        self.startButton.place(x=458 * self.size, y=140 * self.size)

        self.newGameButton1 = Button(self, text=('十三' if self.mode_num == 9 else '九') + '路棋', height=1, width=10,
                                     font=GAME_BUTTON_FONT, command=self.newgame1)
        self.newGameButton1.place(x=458 * self.size, y=175 * self.size)
        self.newGameButton2 = Button(self, text=('十三' if self.mode_num == 19 else '十九') + '路棋', height=1, width=10,
                                     font=GAME_BUTTON_FONT, command=self.newgame2)
        self.newGameButton2.place(x=458 * self.size, y=210 * self.size)

        self.passmeButton = Button(self, text='弃一手', height=1, width=10, font=GAME_BUTTON_FONT, command=self.passme)
        self.passmeButton.place(x=458 * self.size, y=245 * self.size)

        self.regretButton = Button(self, text='悔棋', height=1, width=10, font=GAME_BUTTON_FONT, command=self.regret)
        self.regretButton.place(x=458 * self.size, y=280 * self.size)

        # 初始悔棋按钮禁用
        self.regretButton['state'] = DISABLED

        self.replayButton = Button(self, text='重新开始', height=1, width=10, font=GAME_BUTTON_FONT, command=self.reload)
        self.replayButton.place(x=458 * self.size, y=315 * self.size)

        self.quitButton = Button(self, text='退出游戏', height=1, width=10, font=GAME_BUTTON_FONT, command=self.quit)
        self.quitButton.place(x=458 * self.size, y=350 * self.size)
            ...
            if (20 * self.size - self.dd * 0.4 < event.x < self.dd * 0.4 + 380 * self.size) and (
                ...
                if self.positions[y][x] == 0:
                    ...
                    if not self.last_2_positions == self.positions:
                        ...
                        # 判断是否属于有气和杀死对方其中之一
                        if len(deadlist) > 0 or self.if_dead([[x, y]], self.present + 1, [x, y]) is False:

                            # 当不重复棋局，且属于有气和杀死对方其中之一时，落下棋子有效
                            if self.regretchance is not 1:
                                self.regretchance += 1
                            else:
                                self.regretButton['state'] = NORMAL
                            self.last_3_positions = copy.deepcopy(self.last_2_positions)
...
def main01():
    ...
    while True:
        ...
        app.title("Yuhan's Game of Go")
        app.iconbitmap('./GO.ico')
        app.mainloop()
        if newApp:
            app.destroy()
        else:
